...
logic [2:0] input_cnt;
logic EDGE_cnt_rst;
typedef enum logic [3:0] {FETCH_INPUT,FENCE_FIND_EDGE,FENCE_CAL_AREA,FIN} FSM_state;
...
DW_sqrt #(23,0)
sqrt1 (.a(polygen_length_before),.root(polygen_length_after));
DW_sqrt #(23,0)
...
logic [9:0] X_reg_debug0;
logic [9:0] X_reg_debug1;
logic [9:0] X_reg_debug2;
logic [9:0] X_reg_debug3;
logic [9:0] X_reg_debug4;
logic [9:0] X_reg_debug5;
logic [9:0] Y_reg_debug0;
logic [9:0] Y_reg_debug1;
logic [9:0] Y_reg_debug2;
logic [9:0] Y_reg_debug3;
logic [9:0] Y_reg_debug4;
logic [9:0] Y_reg_debug5;
assign X_reg_debug0 = X_reg[0];
assign X_reg_debug1 = X_reg[1];
assign X_reg_debug2 = X_reg[2];
assign X_reg_debug3 = X_reg[3];
assign X_reg_debug4 = X_reg[4];
assign X_reg_debug5 = X_reg[5];
assign Y_reg_debug0 = Y_reg[0];
assign Y_reg_debug1 = Y_reg[1];
assign Y_reg_debug2 = Y_reg[2];
assign Y_reg_debug3 = Y_reg[3];
assign Y_reg_debug4 = Y_reg[4];
assign Y_reg_debug5 = Y_reg[5];
always_comb begin
	if (CAL_cnt == 3'd5) begin
		...
		test_area_before1 = ({13'd0,R_reg[5]}+{13'd0,R_reg[0]}+{11'd0,polygen_length_after})*({13'd0,R_reg[0]}+{11'd0,polygen_length_after}-{13'd0,R_reg[5]});
		test_area_before2 = ({13'd0,R_reg[5]}+{11'd0,polygen_length_after}-{13'd0,R_reg[0]})*({13'd0,R_reg[5]}+{13'd0,R_reg[0]}-{11'd0,polygen_length_after});
		...
		test_area		= test_area_after1 * test_area_after2;
	end
	else begin
		...
		polygen_length_before = (delta_X*delta_X)+(delta_Y*delta_Y);
		test_area_before1 = ({13'd0,R_reg[CAL_cnt]}+{13'd0,R_reg[CAL_cnt+1]}+{11'd0,polygen_length_after})*({13'd0,R_reg[CAL_cnt+1]}+{11'd0,polygen_length_after}-{13'd0,R_reg[CAL_cnt]});
		test_area_before2 = ({13'd0,R_reg[CAL_cnt]}+{11'd0,polygen_length_after}-{13'd0,R_reg[CAL_cnt+1]})*({13'd0,R_reg[CAL_cnt]}+{13'd0,R_reg[CAL_cnt+1]}-{11'd0,polygen_length_after});
